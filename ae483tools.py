...
def transform_data_mocap(raw_data):
    ...
    p_inA_ofW = np.array([0., 0., -d_1 - d_2])      # <-- fixed
    ...
    R_inQ_ofA = Rotation.from_euler('ZYX', [psi, theta, phi]).as_matrix()                 # <-- fixed
    ...
    p_inW_ofQ = (-R_inQ_ofW).T @ p_inQ_ofW      # <-- fixed
    ...
    for i in range(len(data['time'])):
        ...
        p_inB_ofA = -(R_inA_ofB).T @ p_inA_ofB
        p_inW_ofA = p_inW_ofB + (R_inW_ofB @ p_inB_ofA)     

        # Replace measurements of (x, y, z) and (phi, theta, psi) from mocap
        data['x'][i], data['y'][i], data['z'][i] = p_inW_ofB[0], p_inW_ofB[1], p_inW_ofB[2]            # <-- FIXME i don't really know about this here
        data['yaw'][i], data['pitch'][i], data['roll'][i] = Rotation.from_matrix(R_inW_ofB).as_euler('ZYX', degrees=False)      # <-- FIXME
